...
router.get('/edit:id', (req, res, next)=> {
    ...
    BusinessList.findByid(id, (err, businessListToEdit)=> {
        ...
        {
            //reflesh list
            res.render('edit', {title: 'Edit Business List', page: businessListToEdit,  displayName: UserDisplayName(req)});
        }
...
router.post('/edit:id', (req, res, next)=> {
    ...
    Businessist.updateOne({_id: id}, updatedBusinessList, (err)=> {
        if(err)
        {
            console.log(err);
            res.end(err);
...
function BusinessList(arg0: { name: any; number: any; address: any; }) {
